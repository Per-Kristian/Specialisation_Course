...
One might notice how this resembles the calculation of Gunetti and Picardi's \cite{gnp} R-measure.
...
with the digraphs present in the enrollment sample, it can be used to estimate the flight times of missing digraphs involving the existing keys in the sample.
The DKOs of all missing digraphs is fed as input, and the estimated (expected) flight times will be produced by the network.
This can then be used to 

%The training of the monograph network is performed by sorting the monographs present in the enrollment sample by their average dwell time.
%The result of this is a \textit{monograph mapping table} containing all monographs' key codes and their relative order.
%The actual average dwell time of each monograph is not included in the mapping table.
