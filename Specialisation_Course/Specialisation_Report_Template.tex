...
After a user has logged into a system, there are often no further checks to see if the \textit{current} user has changed.
If the genuine user forgets to lock the computer or log out before they physically leave, an imposter may seize control of their user account.
This gives the imposter opportunities to perform malicious activities such as identity theft and stealing information which can be leveraged by blackmailing the genuine user.
Therefore, being able to detect such imposters would enable the system to remove their access, hopefully before they were able to able to cause harm.

User authentication systems can use keystroke dynamics to in an attempt to solve this problem. 
One way of doing this is observing the current user's typing patterns while they are using the system, and comparing it to the already known typing patterns of the genuine user.
If the current user types in a sufficiently similar manner to the genuine user, they may continue using the system.
However, if the system does not recognize their patterns, they will be locked out in order to protect the genuine user.

Developing such systems is an active research area and several different methods have been proposed to improve the performance and viability of this authentication method.
Most of these systems analyze the typing behavior \textit{periodically} by recording a large amount of keystrokes and then statistically analyzing the behavior in a retroactive manner.
Another type of system performs checks \textit{continuously} after every recorded keystroke, though they only have a few keystrokes to base every decision on.
The possibility of incorporating mechanisms from \textit{periodic} authentication systems into an existing \textit{continuous} authentication system are explored in this paper.
This is done by means of a state of the art survey, as well as detailed discussions on a selected number of periodic authentication systems.
...
\Cref{chap:gnp} contains a review of a PA system using a statistical approach of which several other researchers have based their own systems upon.
...
Periodic authentication and periodic identification are different in terms of the classification algorithms used, as identification systems need to compare the typing behavior of the current user to every legal user in the system in order to determine \textit{who} it is.
...
\end{itemize}
Researchers have generally performed their own data collection, though at least three studies \cite{davoudi2009, davoudi2010, Solami} used Gunetti and Picardi's \cite{gnp} dataset.
...
For consecutive keystrokes, eleven studies \cite{davoudi2009, davoudi2010, Ahmed, KIM2017, Solami, Janakiraman2007, Monrose, BOURS201236, mondal, monaco, park} considered only digraphs, one considered only trigraphs \cite{900words}, while the rest considered n-graphs with multiple values of n where $n>1$.
...
\section{Performance}
As authors of dynamic authentication systems use different metrics to describe their system's performance, comparing them becomes a difficult task.
We can point out that one system \cite{Solami} achieved an accuracy of 100\%, however this is a performance metric for \textit{identification} systems, meaning that it correctly guesses the identity of the user 100\% of the time.
Moreover, we can highlight that the authors of eight PA systems \cite{gnp, davoudi2009, davoudi2010, Ahmed, hu, KIM2017, sliding, park} reported FMRs of 1\% or lower, which means that they fail to detect imposters only 1\% of the time or less frequently.
Such low FMRs may sound promising, but it is also important to consider FNMR to see how often genuine users are falsely detected as imposters.
Not only that, but there are several other properties of each system that must be taken into consideration in order for performance rates to carry any valuable meaning.
The previous subsections of this chapter describe some of those properties.
Therefore, all the mentioned systems are summarized in \Cref{tab:state} to provide an overview of the state of the art.
...
\caption{Summary of state of the art in PA and CA systems. In the database (DB) column, systems marked with a star * have released their database to the public. This table is also presented in my IMT4205 Research Project Planning report \cite{nilsenRPP}.}
\label{tab:state}
\end{table}
